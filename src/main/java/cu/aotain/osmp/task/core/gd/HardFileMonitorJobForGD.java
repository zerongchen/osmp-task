...
public class HardFileMonitorJobForGD extends AbsHardFileMonitorJob {
	...
	public void dealFiles(MonitorResult monitorResult) {
		...
		// 如果存在未正常关闭的999文件，已999异常
		writer999.closeAllWriter("999"); // 999	其他异常（处理中）
	}
	...
	private void dealIcosFiles(File verifiFile, List<File> dataFiles, FileWriter999 writer) {
		...
		/*
		 * 写999样例 File file = dataFiles.get(0); //writer.append(file.getName(),
		 * "this is a test error"); writer.closeWriter(file.getName(), "100");
		 */
		IcosSystemService icosSystemService = ContextUtil.getContext().getBean("icosSystemServiceImp",
				IcosSystemService.class);
	...
	public static class FileWriter999 {
		...
		public void append(String fileName, String line) {
			...
			try {
				...
				logger.error("999 file write fail : path is " + FileWriterManager.getFile(key).getAbsolutePath(), e);
			}
		}
		...
		public void closeWriter(String fileName, String resultCode) {
			...
			try {
				...
				File file = FileWriterManager.getFile(key);
				if(file == null){
					logger.error("999 file write close cache file fail : path is " + fileName, e);
				}else{
					logger.error("999 file write close cache file fail : path is " + file.getAbsolutePath(), e);
				}
			}
		}
		 ...
		 * @param resultCode
		 */
		public void closeAllWriter(String resultCode) {
			if (this.mr.getDataFileList() != null) {
				for (File file : this.mr.getDataFileList()) {
					closeWriter(file.getName(), resultCode);
				}
			}
		}
	}
